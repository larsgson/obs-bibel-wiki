...
const Footer = () => {
  ...
  useEffect(() => {
    ...
    const restorePos = async (obj) => {
      await apiObjGetStorage(obj, "mSec")
        .then((value) => {
          if (value == null) {
      ...
      await apiObjGetStorage(obj, "mSecDur")
        .then((dur) => {
  ...
  if (curPlay != null) {
    ...
    if (videoFound) {
      ...
      if (!isEmptyObj(bibleObj)) {
        ...
        let idStr = pad(id);
        let curFName;
        if (curSerie.audioBible_de_ML) {
          let bBookId = fnameList_DE_ML[bk];
          if (id < 100) {
            idStr = "0" + pad(id);
          }
          curFName = `${curSerie.curPath}${idStr}-${bBookId}_Kapitel-${idStr}.mp3`;
  ...
  if (locURL?.length > 0) {
      ...
      >
        ...
        <div
          style={{
            ...
            height: "80%",
            width: "100%",
            maxWidth: isFB ? 450 : width,
          }}
        >
          <Fab size="small" onClick={closeFooter} style={btnStyle}>
            <NavClose />
          </Fab>
          {videoFound && (
            <VideoPlayer
    ...
    return <footer id="footer" style={{ display: "none" }}></footer>;
  }
};
...
export const MediaPlayer = (props) => {
  ...
  const handlePlaying = (cur) => {
    if (!isPaused) {
      if (cur != null && cur.position != null && isWaitingForPlayInfo) {
      ...
      if (props.onPlaying)
        props.onPlaying({ position: cur.position, duration: cur.duration });
      setCurPos(cur);
    }
  };
